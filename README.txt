...
2. Install Python3 from python.org . Any version of Python 3 is fine. This plugin uses Unreal's built-in Python plugin which runs Python 3.7. This step is necessary to download the libraries used by the plugin.
...
SETUP:
1. Optionally clone https://github.com/proletariatgames/PMXlsxImporterDemo for an example class and data.
1. Create a subclass of UPMXlsxDataAsset. See UPMXlsxImporterDemoTestData in PMXlsxImporterDemo as an example.
2. Add
Meta=(ImportFromXLSX)
to any UPROPERTY in your subclass that should be filled in from an XLSX file
3. Create an XLSX file with your data. See PMXlsxImporterDemo/Content/test.xlsx as an example. The "Name" column will be the name of each data asset, and each other column should be the C++ name of a UPROPERTY marked with Meta=(ImportFromXLSX).
4. In Edit->Project Settings->Asset Manager, add your subclass type as a PrimaryAssetType. You may also want to check "Should Guess Type and Name in Editor" so that the editor can determine names without you needing to manually implement UDataAsset::GetPrimaryAssetId() on each of your UPMXlsxDataAsset subclasses.
5. In Edit->Project Settings->XLSX Import, add an entry, then select your data asset type, XLSX file, worksheet name, and output dir. Note that output dir should be contain only data assets generated by this plugin. The plugin will attempt to delete assets that are not listed in the XLSX file under the assumption that they have been removed from the XLSX file.
6a. Click the "Import XLSX" button on the toolbar. This will show a dialog that allows you to import all XLSX files checked out in your version control, all XLSX files listed in XLSX Import settings, or a specific file. Look at the Output Log to see if there were any errors during the import run.
6b. Alternatively, importing XLSX files can be done via a commandlet. Add this argument to import all XLSX files without opening the editor GUI:
-run=PMXlsxImporter
...
Please consider donating to Proletariat's annual Extra Life charity marathon in November. You can do that by visiting https://www.extra-life.org/ and searching for Proletariat's team.